---
title: "InteroceptionScale - Study 1: Data Analysis"
editor: source
editor_options: 
...
items <- select(df, contains("_Q")) # 120
...
### Factor Retention 
- optimal number of factors to extract 1, 17 (then 29, 101)
...
fa <- parameters::factor_analysis(items, n = 1, scores = "tenBerge", sort = TRUE)
plot(fa)

model_performance(fa) |>display()
summary(fa) |> display()
fa |> display(threshold = 0.3, sort = TRUE)
...
fa <- parameters::factor_analysis(items, n = 17, rotation = "oblimin", scores = "tenBerge", sort = TRUE, cor = "poly")
plot(fa)

model_performance(fa) |>display()
summary(fa) |> display()
fa |> display(threshold = 0.4, sort = TRUE)
# 
# - removal of items with primary loadings < .40 (31 items)
# - removal of items with salient cross-loadings (1 item)
# -  exclusion of factors defined by only two items (4 items)
# -  retention of the three highest-loading items per factor where applicable (39 items removed)

```

```{r}
items_fa <- select(items_fa,
  "Sensitivity_Genital_Q1", "Nociception_Genital_Q1", "Sexual_Genital_Q1",
  "Accuracy_State_Q2", "Accuracy_SkinThermo_Q2", "Accuracy_State_Q1",
  "Nociception_Cardiac_Q2", "Nociception_Respiratory_Q1", "Nociception_Respiratory_Q2",
  "Sensitivity_Respiratory_Q4", "Sensitivity_Respiratory_Q3", "Sensitivity_Respiratory_Q1",
  "Sensitivity_Gastric_Q1", "Sensitivity_Gastric_Q2", "Sensitivity_Gastric_Q3",
  "Anxious_Respiratory_Q1", "Anxious_Cardiac_Q1", "Sensitivity_Cardiac_Q7",
  "Sensitivity_SkinThermo_Q7", "Sensitivity_SkinThermo_Q2", "Sensitivity_SkinThermo_Q1",
  "Sensitivity_ColonBladder_Q3", "Sensitivity_ColonBladder_Q4", "Sensitivity_Gastric_Q5",
  "Nociception_ColonBladder_Q1", "Sensitivity_State_Q4", "Sensitivity_State_Q1",
  "Sexual_ColonBladder_Q2", "Sexual_ColonBladder_Q1", "Sexual_Gastric_Q1",
  "Sexual_SkinThermo_Q1", "Sexual_Respiratory_Q1", "Sexual_Cardiac_Q1",
  "Sensitivity_Cardiac_Q1", "Sensitivity_Cardiac_Q3", "Sensitivity_Cardiac_Q4",
  "Accuracy_ColonBladder_Q1", "Accuracy_ColonBladder_Q5", "Confusion_ColonBladder_Q1",
  "Accuracy_Gastric_Q4", "Accuracy_Gastric_Q6", "Confusion_Gastric_Q2",
  "Sensitivity_Respiratory_Q5", "Sensitivity_Cardiac_Q5", "Sensitivity_Respiratory_Q6"
)

fa <- parameters::factor_analysis(items_fa, n = 15, rotation = "oblimin", scores = "tenBerge", sort = TRUE, cor = "poly")
plot(fa)
...
```{r}
items <- select(items, -all_of(toremove1)) # 116
...
We removed `r length(toremove1)` items (`r paste(toremove1, collapse=", ")`) based on Unique Variable Analysis (UVA).
...
We removed `r length(toremove2)` items (`r paste(toremove2, collapse=", ")`) that had a low stability (< 0.8).
...
We removed `r length(toremove2b)` items (`r paste(toremove2b, collapse=", ")`) that were in clusters with less than 3 items after the first removal.
...
```{r}
items <- select(items, -all_of(toremove2)) # 63
...
```{r}
items <- select(items, -all_of(toremove3)) # 51
...
We kept the 3 items with the highest loading in their lower-level structure, removing `r length(toremove3)` items (`r paste(toremove3, collapse=", ")`).

## Structure Analysis (Final Selection)
...
gt::gtsave(gt::cols_hide(tab1, `|`), "figures/table1.html", inline_css = TRUE) # Save it and reload it otherwise formatting not ideal
...
rez <- EGAnet::network.compare(byfacet[colnames(ega3$lower_order$boot.wc)], 
                               bymodality[colnames(ega3$lower_order$boot.wc)], 
                               model = "glasso", seed=3)
