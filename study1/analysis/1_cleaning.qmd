...
p_eth <- df |>
  ...
  theme_modern(axis.title.space = 15) +
  scale_fill_manual(values = c("East Asian"="#0571B0","Mixed"= "#d01c8b", "White"="#FDB863", "Black"="#E66101", "persian" =  "#ca0020", "South Asian"="#5E3C99", "Hispanic/Latino"="#92C5DE")) +
  theme(
    plot.title = element_text(size = rel(1.2), face = "bold", hjust = 0),
    plot.subtitle = element_text(size = rel(1.2), vjust = 7),
    axis.text.y = element_text(size = rel(1.1)),
...
p_country <- dplyr::select(df, region = Country) |>
  ...
  scale_fill_gradientn(colors = c("#E66101", "#ca0020", "#d01c8b")) +
  labs(fill = "N") +
  theme_void() +
...
p_feed <- estimate_density(df$Experiment_Enjoyment) |>
  ...
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0)) +
  labs(title = "Experiment Enjoyment and Condition", y = "Number of Participants", color = NULL, x = "Ratings of enjoyment on a scale 1-5", subtitle = "Distribution of Participants' Experiment Enjoyment and Condition") +
