...
**TODO: Add feedback comments table**
...
We removed `r length(outliers$attentionchecks)` (`insight::format_percent(length(outliers$attentionchecks) / nrow(df))`) participants for having failed at least 1 attention check (out of 8). 
...
**TODO: Add a little paragraph explaining what it is etc.**
We removed `r length(outliers$distance)` (`insight::format_percent(length(outliers$distance) / nrow(df))`) participants based on multivariate distance. 
