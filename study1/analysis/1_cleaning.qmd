...
parameters::parameters(glm(Outlier_AttentionCheck ~ poly(Experiment_Duration, 2), data = dfoutlier, family = "binomial"))
parameters::parameters(glm(Distance_LOF ~ poly(Experiment_Duration, 2), data = dfoutlier))
```
...
**TODO**
- The age plot code should be streamlined (there is no need to compute estimate_density)
- Education: add in the preprocessing a manual reassignment of some of the "Other" manual inputs to our categories (and put Other if not in the categories)
- We can merge the edu and student plot by having student be a "color" aesthethic 
- Make Pie chart for ethnicity (see doggonogo for example). 
- Ethnicity: manually check the others and either assign them to the correct category or put them in "Other"
...
p_age <- estimate_density(df$Age) |>
  ...
  ggplot(aes(x = x)) +
  geom_histogram(data=df, aes(x = Age, fill=Gender), bins=28, position="identity", alpha=0.5) +
