...
colors <- c(Mint="#00BCD4", IAS="#F44336", MAIA="#FFAD04",  iMAIA="#FF7811", BPQ="#795548")
```
...
```{r}
#EGAnet::genTEFI(ega1$EGA)
# cfa <- EGAnet::CFA(ega3$EGA, 
#             items[colnames(ega3$lower_order$boot.wc)],
#             estimator = "WLSMV")

#dimensionStability(ega1)
```
...
df_clusters <- df |>
  select(Participant, Age, Gender, starts_with("MINT")) |>
  filter(Gender!="Other") |>
  mutate(
    SexS = rowMeans(pick(contains("SexS")), na.rm = TRUE),
    Derm = rowMeans(pick(contains("Derm")), na.rm = TRUE),
    Card = rowMeans(pick(contains("Card")), na.rm = TRUE),
    Sati = rowMeans(pick(contains("Sati")), na.rm = TRUE),
    Gast = rowMeans(pick(contains("Gast")), na.rm = TRUE),
    Olfa = rowMeans(pick(contains("Olfa")), na.rm = TRUE),
    Resp = rowMeans(pick(contains("Resp")), na.rm = TRUE),
    RelA = rowMeans(pick(contains("RelA")), na.rm = TRUE),
    Urin = rowMeans(pick(contains("Urin")), na.rm = TRUE),
    ExAc = rowMeans(pick(contains("ExAc")), na.rm = TRUE),
    CaCo = rowMeans(pick(contains("CaCo")), na.rm = TRUE)
  ) |>
  select(!starts_with("MINT"))
```

```{r}
df_long <- df_clusters |>
  pivot_longer(
    cols = -c(Participant, Age, Gender),
    names_to = "Cluster",
    values_to = "Score") |>
   mutate(
...
ggplot(df_long, aes(x = Cluster, y = Score)) +
  stat_halfeye(
    data = filter(df_long, Gender == "Male"),
    side = "left",
    adjust = 1,
    fill = "#8DBAF0",
    colour = "#050252",
    ...
    point_size = 3) +
  stat_halfeye(
    data = filter(df_long, Gender == "Female"),
    side = "right",
    adjust = 1,
    fill = "#FAA946",
    colour = "#D66200",
...
ggplot(df_long, aes(x = Age,  y = Score))+
      geom_point() +
  facet_wrap(~Cluster)
...
df_art <- read.csv("https://raw.githubusercontent.com/RealityBending/FakeArt/refs/heads/main/data/rawdata_participants.csv") |>
  rename(MINT_Urin_1 = MINT_Urin_13,
         MINT_Urin_2 = MINT_Urin_14,
         MINT_Urin_3 = MINT_Urin_15,
         MINT_CaCo_1 = MINT_CaCo_10,
         MINT_CaCo_2 = MINT_CaCo_11,
         MINT_CaCo_3 = MINT_CaCo_12,
         # MINT_CaNo_1 = MINT_Deficit_CaNo_7,
         # MINT_CaNo_2 = MINT_Deficit_CaNo_8,
         # MINT_CaNo_3 = MINT_Deficit_CaNo_9,
         MINT_Olfa_1 = MINT_Olfa_22,
         MINT_Olfa_2 = MINT_Olfa_23,
         MINT_Olfa_3 = MINT_Olfa_24,
         MINT_Sati_1 = MINT_Sati_19,
         MINT_Sati_2 = MINT_Sati_20,
         MINT_Sati_3 = MINT_Sati_21,
         MINT_SexS_1 = MINT_SexS_7,
         MINT_SexS_2 = MINT_SexS_8,
         MINT_SexS_3 = MINT_SexS_9,
         # MINT_SexO_1 = MINT_Awareness_SexO_22,
         # MINT_SexO_2 = MINT_Awareness_SexO_23,
         # MINT_SexO_3 = MINT_Awareness_SexO_24,
         # MINT_UrSe_1 = MINT_Awareness_UrSe_25,
         # MINT_UrSe_2 = MINT_Awareness_UrSe_26,
         # MINT_UrSe_3 = MINT_Awareness_UrSe_27,
         MINT_RelA_1 = MINT_RelA_4,
         MINT_RelA_2 = MINT_RelA_5,
         MINT_RelA_3 = MINT_RelA_6,
         # MINT_StaS_1 = MINT_Awareness_StaS_31,
         # MINT_StaS_2 = MINT_Awareness_StaS_32,
         # MINT_StaS_3 = MINT_Awareness_StaS_33,
         MINT_ExAc_1 = MINT_ExAc_1,
         MINT_ExAc_2 = MINT_ExAc_2,
         MINT_ExAc_3 = MINT_ExAc_3,
         MINT_Card_1 = MINT_Card_28,
         MINT_Card_2 = MINT_Card_29,
         MINT_Card_3 = MINT_Card_30,
         MINT_Resp_1 = MINT_Resp_25,
         MINT_Resp_2 = MINT_Resp_26,
         MINT_Resp_3 = MINT_Resp_27,
         MINT_Gast_1 = MINT_Gast_31,
         MINT_Gast_2 = MINT_Gast_32,
         MINT_Gast_3 = MINT_Gast_33,
         MINT_Derm_1 = MINT_Derm_16,
         MINT_Derm_2 = MINT_Derm_17,
         MINT_Derm_3 = MINT_Derm_18) |>
...
items_art <- select(df_art, starts_with("MINT_"))
names(items_art) <- str_remove(names(items_art), "MINT_")
```


```{r}
features_art <- data.frame(
  SexS = rowMeans(select(df_art, starts_with("MINT_SexS_")), na.rm = TRUE),
  StaS = rowMeans(select(df_art, starts_with("MINT_StaS_")), na.rm = TRUE),
  SexO = rowMeans(select(df_art, starts_with("MINT_SexO_")), na.rm = TRUE),
  ExaC = rowMeans(select(df_art, starts_with("MINT_ExAc_")), na.rm = TRUE),
  RelA = rowMeans(select(df_art, starts_with("MINT_RelA_")), na.rm = TRUE),
  UrSe = rowMeans(select(df_art, starts_with("MINT_UrSe_")), na.rm = TRUE),
  CaCo = rowMeans(select(df_art, starts_with("MINT_CaCo_")), na.rm = TRUE),
  Urin = rowMeans(select(df_art, starts_with("MINT_Urin_")), na.rm = TRUE),
  Sati = rowMeans(select(df_art, starts_with("MINT_Sati_")), na.rm = TRUE),
  Derm = rowMeans(select(df_art, starts_with("MINT_Derm_")), na.rm = TRUE),
  CaNo = rowMeans(select(df_art, starts_with("MINT_CaNo_")), na.rm = TRUE),
  Olfa = rowMeans(select(df_art, starts_with("MINT_Olfa_")), na.rm = TRUE),
  Resp = rowMeans(select(df_art, starts_with("MINT_Resp_")), na.rm = TRUE),
  Card = rowMeans(select(df_art, starts_with("MINT_Card_")), na.rm = TRUE),
  Gast = rowMeans(select(df_art, starts_with("MINT_Gast_")), na.rm = TRUE)
)

features_art$CardGast = rowMeans(select(features_art, Card, Gast), na.rm = TRUE)
features_art$CardResp = rowMeans(select(features_art, Card, Resp), na.rm = TRUE)
features_art$CardRespGast = rowMeans(select(features_art, Card, Resp, Gast), na.rm = TRUE)
```
...
save(ega_art, file="models/ega_art.RData")
```
...
load("models/ega_art.RData")

# EGAnet::dimensionStability(ega)
itemstability_art <- EGAnet::itemStability(ega_art, IS.plot=FALSE)
p_stab_art <- plot(itemstability_art)
p_stab_art 
...
dimensions <- list(
  ...
  C06 = "Olfa",       # Olfactory Contribution
...
n <- parameters::n_factors(items, type = "FA", rotation = "oblimin")
plot(n)

fa <- parameters::factor_analysis(items, n = 2, rotation = "oblimin", scores = "tenBerge", sort = TRUE, cor = "poly")
plot(fa)

model_performance(fa) |> display()
fa |> display(threshold= 0.2, sort = TRUE)
...
- 9 factors 
```{r}
fa2 <- parameters::factor_analysis(items, n = 9, rotation = "oblimin", scores = "tenBerge", sort = TRUE, cor = "poly")
plot(fa2)

model_performance(fa2) |> display()
fa2 |> display(threshold= 0.2, sort = TRUE)
...
```{r}
clusters <- names(df)[grepl("^MINT_", names(df))]
meta_clusters <- names(df)[grepl("^MINT_Meta_", names(df))]
all_clusters <- unique(c(clusters, meta_clusters))

desc_clusters <- map_dfr(all_clusters, function(var) {
  
  # Compute t-test
  t <- t.test(as.formula(paste(var, "~ Gender")), data = filter(df, Gender != "Other")) |>
    broom::tidy() |>
    mutate(Cluster = var)
  
  # Compute Cohen's d
  d <- effectsize::cohens_d(as.formula(paste(var, "~ Gender")), data = filter(df, Gender != "Other")) |>
    as.data.frame() |>
    round(digits = 2) |>

    mutate(Cluster = var)
  
  df_cluster <- left_join(t, d, by = "Cluster") |>
    select(Cohens_d, p.value, Cluster) 
})

scores_desc <- df |>
  select(Gender, starts_with("MINT_"), starts_with(("MINT_Meta_"))) |>
  filter(Gender != "Other") |>
  pivot_longer( cols = -Gender, names_to = "Cluster", values_to = "Score") |>
...
final_table <- scores_desc |>
  mutate(M_SD = sprintf("%.2f (%.2f)", Mean, SD)) |>
  select(Gender, Cluster, M_SD) |>
  pivot_wider(names_from = Gender, values_from = M_SD) |>
  left_join(desc_clusters,
            by = "Cluster") |>
  arrange(desc(Cohens_d)) |>
  mutate(
    Cohens_d = case_when(
      p.value < .001 ~ sprintf("d = %.2f***", Cohens_d),
      p.value < .01  ~ sprintf("d = %.2f**", Cohens_d ),
      p.value < .05  ~ sprintf("d = %.2f*", Cohens_d),
      TRUE           ~ sprintf("d = %.2f", Cohens_d))) |>
  mutate(
    # Meta_clusters  = case_when(
    #   Cluster == "MINT_Meta_Awareness" ~ "MINT - Awareness",
    #   Cluster == "MINT_Meta_Visceroception"  ~ "MINT - Visceroception",
    #   Cluster == "MINT_Meta_Deficit" ~  "MINT - Deficit"),
    Cluster = recode(
      Cluster,
      "MINT_SexS" = "MINT - Sexual Sensibility",
      "MINT_ExaC" = "MINT - Expulsion Accuracy",
      "MINT_RelA" = "MINT - Relaxation Awareness",
      "MINT_Urin" = "MINT - Urointestinal Inaccuracy",
      "MINT_CaCo" = "MINT - Cardiorespiratory Confusion",
      "MINT_Derm" = "MINT - Dermal Hypersensitivity",
      "MINT_Sati" = "MINT - Satiety Noticing",
      "MINT_Olfa" = "MINT - Olfactory Contribution",
      "MINT_Resp" = "MINT - Respiroception",
      "MINT_Card" = "MINT - Cardioception",
      "MINT_Gast" = "MINT - Gastroception" ,
      "MINT_Meta_Awareness" = "MINT - Awareness",
      "MINT_Meta_Visceroception"  = "MINT - Visceroception",
      "MINT_Meta_Deficit" =  "MINT - Deficit"),
    Cluster = factor(Cluster, levels = c(
      ...
      "MINT - Gastroception",
      "MINT - Deficit",
      "MINT - Cardiorespiratory Confusion",
      "MINT - Dermal Hypersensitivity",
      "MINT - Urointestinal Inaccuracy",
      "MINT - Satiety Noticing",
      "MINT - Olfactory Contribution",
      "MINT - Awareness",
      "MINT - Sexual Sensibility",
      "MINT - Expulsion Accuracy",
      "MINT - Relaxation Awareness"
    ))
    ) |>
  select(!p.value) |>
  arrange(Cluster)
  ...
  gt() |>
   tab_style(
    ...
    locations = cells_body(
      columns = Cluster,
      rows = Cluster %in% c("MINT - Visceroception", "MINT - Deficit", "MINT - Awareness")
    )) |>
  cols_align( align= c("left")) |>
  cols_label(Cohens_d = "Cohen's d",
             Cluster = "") |>
  tab_footnote(footnote = "Cohen's d. Significance: *** p < .001, ** p < .01, * p < .05.") |>
  gt::gtsave("figures/cluster_desc.png", vwidth=1700, expand=0, quiet=TRUE)

```

### Correlations

Note: the comparison against the *metaMint*, *miniMint* and *microMint* short-form variants (and the corresponding *iMAIA*-inclusive model comparisons) has been moved to [3_mint_variants.qmd](3_mint_variants.qmd), so that the figures produced here only contrast the full Mint against the MAIA, BPQ and IAS.
...
p_cormint <- make_cor(mint, qname_x = FALSE, qname_y = FALSE, textsize = 2.8,
...
desc_clusters <- map_dfr(all_clusters, function(var) {
  ...
  df_cluster <- left_join(t, d, by = "Cluster") |>
    select(statistic, parameter, p.value, Cluster, Cohens_d)
})
...
final_table <- scores_desc |>
  ...
  select(Gender, Cluster, M_SD) |>
  pivot_wider(names_from = Gender, values_from = M_SD) |>
  left_join(desc_clusters, by = "Cluster") |>
  arrange(desc(Cohens_d)) |>
  mutate(
     t_df = sprintf("t(%.0f) = %.2f", parameter, statistic),
    p_fmt = ifelse(
      p.value < .001,
      "< .001",
      sub("^0", "", sprintf("%.3f", p.value))
    ),
  ...
  select(!c("p.value","statistic","parameter")) |>
  arrange(Cluster)
  

final_table |>
  select(Cluster, Female, Male, t_df, p_fmt, Cohens_d) |>
  gt() |>
 tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_body(
  ...
  tab_footnote(footnote = gt::md("*t* = Independent samples Welch's *t*-tests. Positive *d* values indicate higher scores in women. df = degrees of freedom.")) |>
  gt::gtsave("figures/table3.png", vwidth=1900, expand=0, quiet=TRUE)
...
p_corintero <- make_cor(select(df, starts_with("MAIA"), IAS, BPQ),
                        mint, qname_y = FALSE, textsize = 2.8,
                        ycol = "#00838F") +
  ...
  for(p in preds) {
    if(length(unique(df[[outcome]])) > 2) {
      if (family == "linear") {
        ...
        models[[p]] <- glm(as.formula(paste0(outcome, " ~ ", p)), data=df, family=family)
      }
    } else {
      models[[p]] <- glm(as.formula(paste0(outcome, " ~ ", p)), data=standardize(df, exclude=outcome), family="binomial")
    }
  }
...
compare_models_lm <- function(outcome="TAS_DIF") {
  ...
  models_full <- list(m_mint=m_mint,
...
make_cor(select(df, starts_with("TAS_")),
         cbind(mint, select(df, starts_with("MAIA"), IAS, BPQ)))
```
...
compare_models_glm <- function(outcome="Disorders_Psychiatric_Mood", title="", subtitle = "") {
  ...
  mods_order <- c("Mint", "MAIA", "iMAIA", "IAS", "BPQ")
...
**Interoceptive Awareness** - Awareness of specific bodily states and internal sensations

1. *Expulsion Accuracy* (ExAc) - Items describing self-perceived accurate awareness of upcoming expulsion sensations (e.g., burp, fart, sneeze).
2. *Relaxation Awareness* (RelA) – Items describing awareness of the bodily state associated with relaxation
3. *Sexual Arousal Sensitivity* (SexS) – Items describing awareness of sensations linked to sexual arousal.


**Interoceptive Deficit** – Confusion, inaccuracy, or delayed awareness of internal bodily signals

1. *Cardiorespiratory Confusion (CaCo)* – Items describing uncertainty about heart and breathing signals.
2. *Urointestinal Inaccuracy (UrIn)* – Items describing uncertainty about urination and defecation signals.
3. *Dermal Hypersensitivity (Derm)* – Items describing heightened sensitivity to touch or materials.
4. *Satiety Noticing (Sati)* – Items describing delayed awareness of hunger and thirst.
5. *Olfactory Contribution (Olfa)* – Items describing the tendency to check body odours to monitor bodily states.

**Visceroception** – Sensitivity to internal visceral sensations

1. *Interoceptive Sensibility* – General sensitivity to internal bodily sensations.
2. *Respiroception (Resp)* – Items describing sensitivity to changes in breathing.
3. *Cardioception (Card)* – Items describing sensitivity to changes in heart activity.
4. *Gastroception (Gast)* – Items describing sensitivity to stomach sensations.
...
make_table1 <- function(preds=preds_dif, mods=mods_dif, group = "Alexithymia", outcome = "TAS - DIF") {
  ...
  if(length(unique(df[[yvar]])) > 2) {
    ...
    r_mint <- cor.test(df[[best_pred_mint$Name[1]]], df[[yvar]])
...
p_cor <- df |> 
  ...
  make_cor(cbind(mint, select(df, starts_with("MAIA"), IAS, BPQ))) +
...
fig3 <- (mods_moodt$p + theme(axis.text.x = element_blank(), axis.title.x = element_blank()) | 
  ...
  plot_annotation(title = "Predictive Performance of Interoceptive Questionnaires", 
       subtitle = "ROC Curves",
       theme = theme(plot.title = element_text(face="bold", hjust = 0.5, size = rel(1.4)), plot.subtitle = element_text(hjust = 0.5, size = rel(1.25))))

fig3

ggsave("figures/fig3.png", fig3, width=15, height=10, dpi=300)
```
