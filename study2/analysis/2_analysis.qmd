...
colors <- c(MINT="#00BCD4", MINTi="#00838F", IAS="#F44336", MAIA="#FF9800", MAIAi="#EF6C00", BPQ="#795548")
```
...
compare_models <- function(outcome="TAS_DIF") {
  ...
  test_all <- test_performance(m_mint, m_minti, m_ias, m_maia, m_maiai, m_bpq)
  test_all$Predictors <- c(
    paste0(cleanlabels(find_predictors(m_mint)$conditional), collapse = ", "),
    paste0(cleanlabels(find_predictors(m_minti)$conditional), collapse = ", "),
    "", 
    paste0(cleanlabels(find_predictors(m_maia)$conditional), collapse = ", "),
    paste0(cleanlabels(find_predictors(m_maiai)$conditional), collapse = ", "),
    "")
  
  list(test=test,  test_all=test_all, p=p)
}
...
```{r}
compare_predictive <- function(outcome="Disorders_Psychiatric_Mood") {
  ...
  test <- test_performance(m_mint, m_minti, m_ias, m_maia, m_maiai, m_bpq)
  ...
  p <- rbind(
    ...
    mutate(as.data.frame(performance_roc(m_bpq)), Predictor = "BPQ")
      ...
      geom_label(data=data.frame(
        label = c(get_auc(m_mint), get_auc(m_minti), get_auc(m_ias), get_auc(m_maia), get_auc(m_maiai), get_auc(m_bpq)),
        Predictor = c("MINT", "MINTi", "IAS", "MAIA", "MAIAi", "BPQ"),
        x = 0.25,
        y = c(0.65, 0.60, 0.55, 0.45, 0.35, 0.20)), 
        aes(x=x, y=y, label=label), color=colors) +
  ...
  list(m_mint=m_mint, m_ias=m_ias, m_maia=m_maia, m_maiai=m_maiai, test=test, p=p)
}
```
...
model <- glm(as.formula(paste0("Wearables_Heart ~ .")),
              data = df |>
