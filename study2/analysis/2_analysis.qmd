...
final_table |>
  ...
  cols_label(Cohens_d = "Cohen's d",
             Cluster = "") |>
  tab_footnote(footnote = "Cohen's d. Significance: *** p < .001, ** p < .01, * p < .05.") |>
  gt::gtsave("figures/table3.png", vwidth=1700, expand=0, quiet=TRUE)
...
fig3 <- (mods_moodt$p + theme(axis.text.x = element_blank(), axis.title.x = element_blank()) | 
  ...
  plot_annotation(title = "Predictive Performance of Interoceptive Questionnaires", 
       subtitle = "ROC Curves",
       theme = theme(plot.title = element_text(face="bold", hjust = 0.5, size = rel(1.4)), plot.subtitle = element_text(hjust = 0.5, size = rel(1))))
