...
#### Meta/ full 
```{r}
meta_MINT_Awareness <- df[, grepl("^MINT_Awareness_(ExAc|RelA|SexS)_", names(df))]
meta_MINT_Deficit <- df[, grepl("^MINT_Deficit_(CaCo|Urin|Derm|Sati|Olfa)_", names(df))]
meta_MINT_Visceroception <- df[, grepl("^MINT_Sensitivity_(Card|Resp|Gast)_", names(df))]

psych::omega(meta_MINT_Awareness, nfactors = 1)
psych::omega(meta_MINT_Deficit, nfactors = 1)
psych::omega(meta_MINT_Visceroception, nfactors = 1)
```

#### Mini

```{r}
mini_MINT_Awareness <- df[, grepl("^MINT_Awareness_(ExAc|RelA)_", names(df))]

mini_MINT_Deficit <- df[, grepl("^MINT_Deficit_(CaCo|Urin)_", names(df))]

mini_MINT_Visceroception <- df[, grepl("^MINT_Sensitivity_(Card|Resp)_", names(df))]

psych::omega(mini_MINT_Awareness, nfactors = 1)
psych::omega(mini_MINT_Deficit, nfactors = 1)
psych::omega(mini_MINT_Visceroception, nfactors = 1)
```
...
```{r}
Macro_MINT_Awareness <- df[, grepl("^MINT_(Awareness_ExAc|Deficit_CaCo|Sensitivity_Resp)_", names(df))]

psych::omega(Macro_MINT_Awareness, nfactors = 1)
...
tas <- df |>
  ...
  mutate(TAS_DIF_9 = 6 - TAS_DIF_9,
         TAS_DIF_13 = 6- TAS_DIF_13 , 
         TAS_EOT_15 = 6- TAS_EOT_15,
         TAS_EOT_18 = 6- TAS_EOT_18,
         TAS_EOT_19 = 6- TAS_EOT_19)

tas_eot <- tas[, grepl("^TAS_EOT_", names(tas))]
tas_ddf <-  tas[, grepl("^TAS_DDF_", names(tas))]
tas_dif <-  tas[, grepl("^TAS_DIF_", names(tas))]

psych::omega(tas_eot, nfactors = 1)
psych::omega(tas_ddf, nfactors = 1)
psych::omega(tas_dif, nfactors = 1)
```

### ERS
```{r}
ers_a <- df[, grepl("^ERS_Arousal_", names(df))]
ers_s <-  df[, grepl("^ERS_Sensitivity_", names(df))]
ers_p <-  df[, grepl("^ERS_Persistence_", names(df))]

psych::omega(ers_a, nfactors = 1)
psych::omega(tas_ddf, nfactors = 1)
psych::omega(tas_dif, nfactors = 1)
...
### CERQ
```{r}
cerq_acept <- df[, grepl("^CERQ_Acceptance_", names(df))]
cerq_posref <- df[, grepl("^CERQ_PositiveRefocusing_", names(df))]
cerq_posrea <- df[, grepl("^CERQ_PositiveReappraisal_", names(df))]
cerq_selfbla <- df[, grepl("^CERQ_SelfBlame_", names(df))]
cerq_otherbla <- df[, grepl("^CERQ_OtherBlame_", names(df))]
cerq_rum <- df[, grepl("^CERQ_Rumination_", names(df))]
cerq_cat <- df[, grepl("^CERQ_Catastrophizing_", names(df))]
cerq_refopla <- df[, grepl("^CERQ_RefocusPlanning_", names(df))]
cerq_persp <- df[, grepl("^CERQ_Perspective_", names(df))]

psych::omega(cerq_acept, nfactors = 1)
psych::omega(cerq_posref, nfactors = 1)
psych::omega(cerq_posrea, nfactors = 1)
psych::omega(cerq_selfbla, nfactors = 1)
psych::omega(cerq_otherbla, nfactors = 1)
psych::omega(cerq_rum, nfactors = 1)
psych::omega(cerq_cat, nfactors = 1)
psych::omega(cerq_refopla, nfactors = 1)
psych::omega(cerq_persp, nfactors = 1)
```

### CEFSA
```{r}
cefsa_emo <- df[, grepl("^CEFSA_Emotion_", names(df))]
cefsa_con <- df[, grepl("^CEFSA_Connection_", names(df))]
cefsa_body <- df[, grepl("^CEFSA_Body_", names(df))]
cefsa_rea <- df[, grepl("^CEFSA_Reality_", names(df))]
cefsa_fam <- df[, grepl("^CEFSA_Familiarity_", names(df))]
cefsa_age <- df[, grepl("^CEFSA_Agency_", names(df))]
cefsa_self <- df[, grepl("^CEFSA_Self_", names(df))]


psych::omega(cefsa_emo, nfactors = 1)
psych::omega(cefsa_con, nfactors = 1)
psych::omega(cefsa_body, nfactors = 1)
psych::omega(cefsa_rea, nfactors = 1)
psych::omega(cefsa_fam, nfactors = 1)
psych::omega(cefsa_age, nfactors = 1)
psych::omega(cefsa_self, nfactors = 1)
```
...
pi_alive <- df[grep("^PI18.*A_", names(df))]
pi_safe <- df[grep("^PI18.*GS", names(df))]
pi_enticing <- df[grep("^PI18.*GE", names(df))]
pi_good <- df[c(grep("^PI18.*GS", names(df)), grep("^PI18.*GE", names(df)))]

psych::omega(pi_alive, nfactors = 1)
psych::omega(pi_safe, nfactors = 1)
psych::omega(pi_enticing, nfactors = 1)
psych::omega(pi_good, nfactors = 1)
```

### PHQ
```{r}
phq_a <- df[grep("^PHQ4_Anxiety", names(df))]
phq_d <- df[grep("^PHQ4_Depression", names(df))]


psych::omega(phq_a, nfactors = 1)
psych::omega(phq_d, nfactors = 1)
```
...
#There are 5 items that are negatively keyed (items 9, 13, 15, 18 and 19) in the TAS
