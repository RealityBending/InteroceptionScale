...
**Other questionnaires include the Body Consciousness Questionnaire [BCS, @miller1981consciousness], the  Self-Awareness Questionnaire [SAQ, @longarzo2015relationships], the Interoceptive Sensitivity and Attention Questionnaire [ISAQ , @bogaerts2022interoceptive];  the Eating Disorder Inventory [EDI-IA, @garner1983development] the Interoceptive Confusion Questionnaire [ICQ, @Brewer2016] and the Interoceptive Attention Scale [IATS, @GABRIELE2022108243]. For an extensive review of measures used in the field of interoception see @desmedt2022measures and @mmehling2009body.**
...
To address this gap, we developed and **assessed the Multimodal Interoception Questionnaire (Mint)**, a new questionnaire that systematically addresses the above limitations. In Study 1, we describe the **XXX** development of the Mint, using a "modality-by-context" framework to generate a comprehensive item pool, which was then refined using cutting-edge psychometric network analysis.

**Network psychometrics is an emerging framework for studying relationships among psychological variables by representing them as a network structure [@golino2017exploratory]. In this approach, psychological variables (e.g., traits or questionnaire items) are modelled as nodes, while the statistical relationships between them are represented as edges connecting the nodes. These edges can be weighted to reflect the strength and direction of associations between variables. Beyond descriptive modelling, network psychometrics has increasingly been applied to scale development and dimensionality assessment [@GOLINO201754; @christensen2020psychometric; @magnavita2022development; @wongvorachan2025item]. When analysing psychological measurement data, a central question concerns whether observed variables organise into groupings that reflect underlying constructs. Within a latent variable framework, items measuring the same construct are expected to show stronger mutual associations, resulting in cohesive clusters within a network representation. Accordingly, clusters of interconnected nodes in psychometric networks have been proposed to correspond to latent variables [@golino2017exploratory].**
