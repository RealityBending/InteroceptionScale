...
\par{ }
\par{     \begin{tcolorbox}[enhanced jigsaw, toprule=.15mm, leftrule=.75mm, colback=white, bottomrule=.15mm, rightrule=.15mm, arc=.35mm, left=2mm, opacityback=0, colframe=quarto-callout-note-color-frame, breakable]
...
Eating Disorder Inventory (EDI-IA,
\citeproc{ref-garner1983development}{Garner et al., 1983}) the
...
the \textbf{XXX} development of the Mint, using a
``modality-by-context'' framework to generate a comprehensive item pool,
which was then refined using cutting-edge psychometric network analysis.
...
questionnaires, we adopted a \textbf{XXX}, theoretically agnostic
approach. Rather than imposing a pre-defined theoretical structure on
...
Item generation was guided by a ``modality-by-context'' grid.
\textbf{Specifically, item development proceeded through a structured
process aimed at maximising coverage and consistency across
interoceptive modalities and experiential contexts. Existing
interoception questionnaires were first systematically reviewed to
characterise the current measurement landscape, including content
coverage, item phrasing, and response formats. Insights from this
review, together with relevant theoretical literature and expert
knowledge, informed the construction of the modality-by-context grid,
which was designed to capture core principles of interoceptive
...
facets\^{}{[}The initial item list is available at \emph{{[}MASKED FOR
REVIEW{]}} . Items were generated through conversations between the
...
awareness of interoceptive processes. Furthermore, our \textbf{XXX}
approach revealed intriguing facets, such as ``Olfactory Contribution''.
...
the higher-order meta-clusters can be computed from their corresponding
lower-order clusters. Correspondingly, the high-order meta-clusters of
the mint showed good to excellent internal consistency: Interoceptive
...
more broadly to refer to interoceptive signals that are accessible to
conscious self-report (\citeproc{ref-khalsa2018interoception}{Khalsa et
al., 2018}). The items in this cluster therefore capture perceived
awareness of bodily states such as relaxation (RelA), sensations
associated with sexual arousal (SexS), and impending expulsive events
(ExAc; e.g., burping, sneezing, or flatulence), reflecting an
individual's perceived ability to recognise these bodily states. The
``Interoceptive Deficit'' cluster reflects difficulties, uncertainty, or
inaccuracies in recognising internal bodily signals. Items in this
cluster capture experiences such as confusion about cardiorespiratory
sensations (CaCo), uncertainty about urinary or gastrointestinal needs
(UrIn), delayed recognition of hunger or thirst (Sati), contribution of
external cues such as checking body odours (Olfa), and heightened
sensitivity to dermal stimulation (Derm). Finally, ``Visceroception''
corresponds broadly to the concept of interoceptive sensibility
described in Khalsa et al.
...
particularly given that interoceptive processes vary according to
demographic factors such as gender, culture, and sex (e.g.,
\citeproc{ref-ma2024demographic}{Ma-Kellams et al., 2024}). For example,
the Japanese validation of the MAIA-2
...
\bibitem[\citeproctext]{ref-garner1983development}
